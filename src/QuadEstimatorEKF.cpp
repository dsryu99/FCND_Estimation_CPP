...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) = curState(3) + accel_iner.x * dt;
  predictedState(4) = curState(4) + accel_iner.y * dt;
  predictedState(5) = curState(5) + accel_iner.z * dt - CONST_GRAVITY * dt;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  MatrixXf mat = MatrixXf::Identity(QUAD_EKF_NUM_STATES - 1, QUAD_EKF_NUM_STATES - 1);
  hPrime.topLeftCorner(QUAD_EKF_NUM_STATES - 1, QUAD_EKF_NUM_STATES - 1) = mat;
